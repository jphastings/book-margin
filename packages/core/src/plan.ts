...
/** One annotation in a plan: the raw highlight, plus the record/rkey when the book resolved. */
 ...
 * resolve each to an ISBN, and map every annotation to a record with its
 * deterministic rkey. Books that don't resolve come back with `isbn13`
 * undefined and entries that carry only the raw highlight. This is the basis
 * for both the CLI sync and the web review UI.
 */
export async function planSync(
  ...
  for (const group of groupByBook(highlights)) {
    plan.push(
      await planBook(group.book, group.highlights, { ...options, resolve: resolveOptions }),
    );
  }
  return plan;
}
...
export async function planBook(
  ...
  if (!resolved) {
    return { book, entries: highlights.map((highlight) => ({ highlight })), collapsed: 0 };
  }
  ...
  for (const highlight of highlights) {
    ...
    seen.add(rkey);
    entries.push({ highlight, rkey, note });
  }
